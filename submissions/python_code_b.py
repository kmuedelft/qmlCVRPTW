...
for i in Nodes:
    if i != 0:
        con2[i] = m.addConstr(
            quicksum(z[i,v] for v in Vehicles) == 1, 'con2[' + str(i) + ']'
        )
...
for v in Vehicles:
    con3[v] = m.addConstr(
        quicksum(edge[0, i, v] for i in Nodes if i != 0) <= 1, 'con3[' + str(v) + ']'
    )
    con4[v] = m.addConstr(
        quicksum(edge[i, 0, v] for i in Nodes if i != 0) <= 1, 'con4[' + str(v) + ']'
    )
...
for v in Vehicles:
    con5[v] = m.addConstr(
        quicksum(q[i]*z[i,v] for i in Nodes) <= Q, 'con5[' + str(v) + ']'
    )
...
for i in Nodes:
    for v in Vehicles:
        con7[i,v] = m.addConstr(
            service_start[i,v] >= earliest_time[i], 'con7[' + str(i) + ',' + str(v) + ']'
            )
        con8[i,v] = m.addConstr(
            service_start[i,v] <= latest_time[i], 'con8[' + str(i) + ',' + str(v) + ']'
            )
...
for i in Nodes:
    for j in Nodes:
        for v in Vehicles:
            if i != j and j != 0:
                con9[i,j,v] = m.addConstr(
                    service_start[j,v] >= service_start[i,v] + service_duration[i] + distance[i,j] - BIGM*(1-edge[i,j,v]), 'con9[' + str(i) + ',' + str(j) + ',' + str(v) + ']'
                    )
