...
with open('data_large_multiTW_diff.txt', 'r') as f:
    data = []
    for line in f:
        # Skip empty lines
        if not line.strip():
...
for entry in data:
    ...
    entry['num_time_windows'] = max_time_windows  # Update num_time_windows to max_time_windows
...
for i in Nodes:
    for v in Vehicles:
        for w in range(len(time_windows[int(i)])):
            fraq[i, v, w] = m.addVar(vtype=GRB.CONTINUOUS, lb=0, ub=1, name='fraq_%s_%s_%s' % (i, v, w))

# if time window w is selected for node i
z = {}
for i in Nodes:
    for w in range(len(time_windows[int(i)])):
        z[i,w] = m.addVar(vtype=GRB.BINARY, lb = 0, name='z_%s_%s' % (i, w))
...
for i in Nodes:
    for v in Vehicles:
        con2[i,j,v] = m.addConstr(
            quicksum(edge[i,j,v] for j in Nodes) >= quicksum(fraq[i, v, w] for w in range(len(time_windows[int(i)]))), 'con2[' + str(i) + ',' + str(j) + ',' + str(v) + ']'
        )

# complete demand picked up for each node
con3 = {}
for i in Nodes:
    if i != 0:
        con3[i,v,w] = m.addConstr(
            quicksum(fraq[i,v,w] for v in Vehicles for w in range(len(time_windows[int(i)]))) == 1, 'con3[' + str(i) + ',' + str(v) + ',' + str(w) + ']'
        )
...
for v in Vehicles:
    con6[v] = m.addConstr(
        quicksum(q[i]*fraq[i,v,w] for i in Nodes for w in range(len(time_windows[int(i)]))) <= Q, 'con6[' + str(v) + ']'
    )
...
# if node i is note visited in w, the vehicle can not pick sth up
con13 = {}
for i in Nodes:
    for w in range(len(time_windows[int(i)])):
        con13[i,w] = m.addConstr(
            z[i,w] >= quicksum(fraq[i,v,w] for v in Vehicles), 'con13[' + str(i) + ',' + str(w) + ']'
            )
...
if m.status == GRB.OPTIMAL:
    ...
    for i in Nodes:
        for v in Vehicles:
            for w in range(len(time_windows[int(i)])):
                if fraq[i, v, w].X > 0:  # z[i, v, w].X gibt den Wert der Variablen nach der Optimierung an
                    a_iw, b_iw = time_windows[int(i)][w]
                    start = service_start[i,v].X
                    print(f"Node {i} is visited by vehicle {v} in time window ({a_iw}, {b_iw} at {start:.2f}).")
else:
...
if m.status == GRB.OPTIMAL:
    ...
    for i, (x, z) in enumerate(zip(xcoord, ycoord)):
        plt.annotate(str(i), (x, z), textcoords="offset points", xytext=(0, 5), ha='center')
    ...
    for v in Vehicles:
        ...
        for (i, j) in sorted_routes:
            load += q[i]*sum(fraq[i, v, w].X for w in range(len(time_windows[int(i)])))
